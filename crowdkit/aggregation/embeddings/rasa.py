...
class RASA(BaseEmbeddingsAggregator):
    ...
    **Step 3**. Estimate the aggregated result. It is the output which embedding is
    ...
    def _aggregate_embeddings(
        data: pd.DataFrame, skills: 'pd.Series[Any]', true_embeddings: Optional['pd.Series[Any]'] = None
    ) -> 'pd.Series[Any]':
    ...
    def _apply(self, data: pd.DataFrame, true_embeddings: Optional['pd.Series[Any]'] = None) -> "RASA":
    ...
    def fit(self, data: pd.DataFrame, true_embeddings: Optional['pd.Series[Any]'] = None) -> "RASA":
