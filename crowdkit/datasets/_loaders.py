...
def load_relevance2(data_dir: Optional[str] = None) -> Tuple[pd.DataFrame, 'pd.Series[Any]']:
...
def load_relevance5(data_dir: Optional[str] = None) -> Tuple[pd.DataFrame, 'pd.Series[Any]']:
...
def load_mscoco(data_dir: Optional[str] = None) -> Tuple[pd.DataFrame, 'pd.Series[Any]']:
    data_name = "mscoco"
...
def load_mscoco_small(data_dir: Optional[str] = None) -> Tuple[pd.DataFrame, 'pd.Series[Any]']:
...
def load_crowdspeech_dataframes(data_path: str) -> Tuple[pd.DataFrame, 'pd.Series[Any]']:
    labels = pd.read_csv(join(data_path, "crowd_labels.csv")).rename(
...
DATA_LOADERS: Dict[
    str,
    Dict[str, Union[str, Callable[[Optional[str]], Tuple[pd.DataFrame, 'pd.Series[Any]']]]],
] = {
    "relevance-2": {
