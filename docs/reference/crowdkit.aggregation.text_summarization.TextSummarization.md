# TextSummarization
`crowdkit.aggregation.text_summarization.TextSummarization`
...
TextSummarization(
    ...
    permutation_aggregator: Optional[BaseAggregator] = None,
    device: str = 'cpu'
)
```
...
**To use this method, you need to install Crowd-Kit with additional dependencies with `pip install crowd-kit[transformers]`**
...
`permutation_aggregator`|**Optional\[BaseAggregator\]**|<p>Text aggregation method to use for aggregating outputs of multiple input permutations if `use_permutations` flag is set. </p><p>Default value: `None`.</p>
...
| Method | Description |
| :------| :-----------|
[fit_predict](crowdkit.aggregation.text_summarization.TextSummarization.fit_predict.md)| None
