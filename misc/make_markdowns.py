...
class GitHubSourceFinder:
    ...
    def __call__(self, definition: BaseDefinition):
        ...
        if definition_module.__name__.split(".")[
            -1
        ] != definition_module.__file__.split("/")[-1].replace(".py", ""):
...
def make_markdowns(
    ...
    for module_name, module in traverse_modules(
        module_root, src_root, skip_modules=skip_modules
    ):
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Module name to import these sources as",
    )
    parser.add_argument(
        "--src-root",
        type=os.path.abspath,
        required=True,
        help="Path to source files to process",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        nargs="*",
        help="Module names to skip when generating markdowns",
    ...
    if args.described_objects:
        ...
        if described_objects is None:
            raise RuntimeError(
                "DESCRIBED_OBJECTS dict should be defined in python file specified in described-objects"
            )
    ...
    make_markdowns(
        ...
        modules_aliases_mapping=args.modules_aliases
        and json.load(open(args.modules_aliases)),
