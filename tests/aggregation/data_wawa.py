...
@pytest.fixture
def toy_labels_result_wawa() -> 'pd.Series[Any]':
    return pd.Series(
        ["no", "yes", "no", "yes", "no"],
...
@pytest.fixture
def toy_skills_result_wawa() -> 'pd.Series[Any]':
    return pd.Series(
        [0.6, 0.8, 1.0, 0.4, 0.8],
...
@pytest.fixture
def simple_labels_result_wawa(simple_ground_truth: 'pd.Series[Any]') -> 'pd.Series[Any]':
    return simple_ground_truth


@pytest.fixture
def simple_skills_result_wawa() -> 'pd.Series[Any]':
    skills = pd.Series(
        {
