...
@pytest.fixture
def result_empty() -> 'pd.Series[Any]':
    return pd.Series([], dtype=np.float64, name="agg_score")


@pytest.fixture
def result_equal() -> 'pd.Series[Any]':
    return pd.Series([1 / 3, 1 / 3, 1 / 3], index=["a", "b", "c"], name="agg_score")


@pytest.fixture
def noisy_bt_result() -> 'pd.Series[Any]':
    return pd.Series(
        [1.0, 1.0, 1.497123058531228e-45],
...
@pytest.fixture
def noisy_bt_result_equal() -> 'pd.Series[Any]':
    return pd.Series(
        [0.6715468044437242, 0.6462882683525435, 0.632947637600415],
        index=pd.Index(["a", "b", "c"], name="label"),
        name="agg_score",
    )
...
@pytest.fixture
def result_iter_0() -> 'pd.Series[Any]':
    return pd.Series([1 / 3, 1 / 3, 1 / 3], index=["a", "b", "c"], name="agg_score")


@pytest.fixture
def result_iter_10() -> 'pd.Series[Any]':
    return pd.Series([0.934, 0.065, 0.0], index=["a", "b", "c"], name="agg_score")


def test_bradley_terry_empty(result_empty: 'pd.Series[Any]', data_empty: pd.DataFrame) -> None:
...
def test_bradley_terry_equal(
    n_iter: int, tol: float, result_equal: 'pd.Series[Any]', data_equal: pd.DataFrame
) -> None:
...
def test_bradley_terry_step_by_step(
    request: Any, data_abc: pd.DataFrame, n_iter: int
) -> None:
...
@pytest.mark.parametrize("n_iter, tol", [(10, 0), (100500, 1e-5)])
def test_noisy_bradley_terry(
    n_iter: int, tol: float, data_abc: pd.DataFrame, noisy_bt_result: 'pd.Series[Any]'
) -> None:
...
def test_noisy_bradley_terry_equal(
    data_equal: pd.DataFrame, noisy_bt_result_equal: 'pd.Series[Any]'
) -> None:
...
def test_zero_iter(
    agg_class: Any, data_equal: pd.DataFrame, result_equal: 'pd.Series[Any]'
) -> None:
