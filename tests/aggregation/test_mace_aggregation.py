...
def test_thetas_on_simple(
    simple_answers_df: pd.DataFrame, thetas_simple: pd.DataFrame
) -> None:
    thetas = (
        MACE(n_restarts=1, n_iter=5, method="vb", random_state=0)
        .fit(simple_answers_df)
        .thetas_
    )
