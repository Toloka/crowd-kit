...
def test_rover_aggregation(rover_toy_result: 'pd.Series[Any]', data_toy: pd.DataFrame) -> None:
...
def test_rover_single_overlap(
    rover_single_overlap_data: pd.DataFrame, rover_single_overlap_result: 'pd.Series[Any]'
) -> None:
    rover = ROVER(tokenizer=lambda x: x.split(" "), detokenizer=lambda x: " ".join(x))
    assert_series_equal(
        rover_single_overlap_result, rover.fit_predict(rover_single_overlap_data)
    )


def test_rover_simple_text(
    simple_text_df: pd.DataFrame, simple_text_result_rover: 'pd.Series[Any]'  # noqa F811
) -> None:
    rover = ROVER(tokenizer=lambda x: x.split(" "), detokenizer=lambda x: " ".join(x))
