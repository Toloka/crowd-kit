...
def test_simple_segmentation_rasa_iter(
    n_iter: int,
    tol: float,
...
@pytest.mark.parametrize("n_iter, tol", [(10, 0), (100500, 1e-5)])
def test_simple_segmentation_em_iter(
    n_iter: int,
    tol: float,
    simple_image_df: pd.DataFrame,
...
def test_zero_iter(
    agg_class: Any, simple_image_df: pd.DataFrame, simple_image_mv_result: 'pd.Series[Any]'
) -> None:
