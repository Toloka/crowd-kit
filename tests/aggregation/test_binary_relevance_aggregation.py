...
from crowdkit.aggregation import GLAD, BinaryRelevance, DawidSkene, MajorityVote, Wawa
...
@pytest.mark.parametrize("aggregator", [MajorityVote(), DawidSkene(), Wawa(), GLAD()])
...
def test_binary_relevance_aggregation_on_toy_data(
    ...
    assert_series_equal(
        binary_relevance_toy_result, mb.fit_predict(data_toy_binary_relevance)
    )


@pytest.mark.parametrize("aggregator", [MajorityVote(), DawidSkene(), Wawa(), GLAD()])
def test_binary_relevance_aggregation_on_empty(
    aggregator: BaseClassificationAggregator,
) -> None:
    mb = BinaryRelevance(aggregator)
    result = mb.fit_predict(pd.DataFrame([], columns=["task", "worker", "label"]))
    assert_series_equal(
        pd.Series(dtype=float, name="agg_label"), result, check_index_type=False
    )
